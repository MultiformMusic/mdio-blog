...
export async function getAllBlogs() {
    ...
    const results = await client.fetch(`*[_type == "blog"] | order(date desc) { ${blogFileds}}`);
    return results;
}
...
export async function getBlogBySlug(slug, preview) {
    ...
    const result = await currentClient.fetch(`*[_type == "blog" && slug.current == $slug]{ ${blogFileds} content[]{..., "asset": asset->} }`, {slug})
